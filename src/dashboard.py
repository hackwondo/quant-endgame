...
def display_local_pdf(file_path):
    # 파일명 뒤에 .pdf가 없으면 자동으로 붙여서 찾아봄
    if not file_path.lower().endswith('.pdf'):
        check_path = file_path + ".pdf"
    else:
        check_path = file_path
        
    full_path = os.path.abspath(check_path)
    
    if not os.path.exists(full_path):
        st.error(f"❌ '{check_path}' 파일을 찾을 수 없습니다.")
        return
...
#
#
#& 'C:\Users\hackwondo\AppData\Local\Programs\Python\Python313\python.exe' -m streamlit run dashboard.py
